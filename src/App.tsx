...
const customRound = (amount: number): number => {
  const lastDigit = Math.floor(amount) % 10;
  const baseAmount = Math.floor(amount / 10) * 10;
  
  if (lastDigit <= 5) {
    return baseAmount; // Redondear hacia abajo (ej: 55 -> 50, 53 -> 50)
  } else {
    return baseAmount + 10; // Redondear hacia arriba (ej: 56 -> 60, 59 -> 60)
  }
};
...
function App() {
  ...
  const calculateFare = useCallback((distanceKm: number, waitingMinutes: number) => {
    ...
    if (selectedTripType.fixedPrice || (selectedTripType.id === 'cristoRey' && selectedSubTrip)) {
      ...
      if (distanceKm > baseDistance) {
        ...
        if (selectedTripType.id === 'cristoRey') {
          // Cristo Rey: $16 por cada km adicional después de 1.5km base
          const extraCost = extraKm * 16;
          baseCost += extraCost;
        } else {
      ...
      return customRound(baseCost + waitingCost);
    }
    ...
    return customRound(fare + (waitingMinutes * RATES.waitingRate));
  }, [selectedTripType, selectedSubTrip]);
        ...
        if (newDistanceMeters > THRESHOLD) {
          ...
          setTripData(prev => {
            ...
            return {
              ...
              cost: calculateFare(adjustedDistance, waitingMinutes)
            };
          });
  ...
  useEffect(() => {
    if (!tripData.isRunning) {
      setTripData(prev => ({
        ...prev,
        cost: customRound(getBasePrice(selectedTripType))
      }));
    }
                  ...
                  <div className="border-t border-gray-600 pt-2 mt-2">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-2xl text-green-400">
                        ${customRound(lastTripSummary.cost)} MXN
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  >
                    ...
                    {tripType.fixedPrice && (
                      <div className="text-green-400 font-bold mt-2">
                        Precio base: ${customRound(tripType.fixedPrice)} MXN
                      </div>
                    )}
                    {tripType.subTrips && (
              ...
              <div className="space-y-3">
                {selectedTripType.subTrips?.map((subTrip) => (
                  ...
                  >
                    ...
                    <div className="text-green-400 font-bold mt-2">
                      Precio: ${customRound(subTrip.fixedPrice)} MXN
                    </div>
                  </button>
                ))}
              </div>
                ...
                <div className="mt-2 bg-gradient-to-r from-blue-800 to-blue-900 text-white p-3 rounded-xl text-center border border-blue-500 shadow-lg">
                  ...
                  {(selectedTripType.fixedPrice || selectedSubTrip) && (
                    <p className="text-green-400 font-bold mt-1">
                      Precio base: ${customRound(selectedSubTrip ? selectedSubTrip.fixedPrice : selectedTripType.fixedPrice!)} MXN + tiempo de espera
                    </p>
                  )}
                </div>
              )}
